...
@pytest.mark.parametrize("total_summaries, events, diff, expected", [
    ([],
    [
...
def test_work(monkeypatch, total_summaries, events, diff, expected):
    base_date=datetime.fromisoformat("2022-05-03T00:00:00+03:00")

    content = Content()

    content.events = [total_summaries, events]
    content.now = base_date
    content.time_bounds = [base_date + timedelta(days=diff)]
